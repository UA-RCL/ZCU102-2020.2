...
typedef struct {
    ...
    XZip_hls_accel_uio_map maps[ MAX_UIO_MAPS ];
} XZip_hls_accel_uio_info;

/***************** Variable Definitions **************************************/
static XZip_hls_accel_uio_info uio_info;
...
static int uio_info_read_name(XZip_hls_accel_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XZip_hls_accel_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XZip_hls_accel_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XZip_hls_accel_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XZip_hls_accel_Initialize(XZip_hls_accel *InstancePtr, const char* InstanceName) {
	XZip_hls_accel_uio_info *InfoPtr = &uio_info;

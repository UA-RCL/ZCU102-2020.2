...
void wrapper_zip_hw(hls::stream<AXI_VAL> &in_stream, hls::stream<AXI_VAL> &out_stream, int size, int op) {
    ...
    VITIS_LOOP_22_1: for (int i = 0; i < 2 * size; i++) {
...
#pragma HLS PIPELINE II=1
 val=in_stream.read();
    ...
    VITIS_LOOP_29_2: for (int i = 0; i < 2 * size; i++) {
#pragma HLS LOOP_TRIPCOUNT min = 2 max = MAX_SIZE
#pragma HLS PIPELINE II=1
 val=in_stream.read();
        B[i] = val.data;
    ...
    VITIS_LOOP_56_3: for (int i = 0; i < 2 * size; i++) {
#pragma HLS LOOP_TRIPCOUNT min = 2 max = MAX_SIZE
#pragma HLS PIPELINE II=1
 val.data = C[i];
        val.last = (i == (2 * size - 1));
...
__attribute__((sdx_kernel("ZIP_HLS_accel", 0))) void ZIP_HLS_accel (hls::stream<AXI_VAL> &INPUT_STREAM, hls::stream<AXI_VAL> &OUTPUT_STREAM, int size, int op){
#pragma HLS TOP name=ZIP_HLS_accel
# 67 "zip_axiwrapper.cpp"

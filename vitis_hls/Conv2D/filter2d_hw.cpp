...
struct window {
    pix_t pix[FILTER_V_SIZE][FILTER_H_SIZE];
};
        ...
        hls::stream<window>  &window_stream)
{
    ...
    pix_t LineBuffer[FILTER_V_SIZE-1][MAX_IMAGE_WIDTH];
		...
		hls::stream<pix_out_t>  &pixel_stream)
{
    ...
    float coeffs[FILTER_V_SIZE][FILTER_H_SIZE];
...
#pragma HLS PIPELINE II=1
            ...
            // Normalize result
            // U8 outpix = MIN(MAX((int(factor * sum)+bias), 0), 255);
            // U8 outpix = MIN(MAX((sum + bias), 0), 255);
            pix_out_t outpix;
            outpix.data = (pix_t)MIN(MAX((sum + bias), CONV_2D_MIN), CONV_2D_MAX);
            outpix.keep = -1;
...
#pragma HLS DATAFLOW
    ...
    ReadFromMem(width, height, stride, coeffs, coeff_stream/*, src, pixel_stream*/);
